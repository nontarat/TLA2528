 ...
 * example
 * TLA2528_Enable_GPIO(GPIO_CFG_GPO2);  // เปิดใช้งาน GPO2
 * TLA2528_Disable_GPIO(GPIO_CFG_GPO2); // ปิดใช้งาน GPO2
 *
 */
uint8_t TLA2528_Read_GPIO_Config(void) {
    return TLA2528_Read_Register(GPIO_CFG_ADDRESS);
}

void TLA2528_Set_GPIO_Config(uint8_t config) {
    TLA2528_Write_Register(GPIO_CFG_ADDRESS, config);
}

void TLA2528_Enable_GPIO(uint8_t gpio_pin) {
    uint8_t current_config = TLA2528_Read_GPIO_Config(); // อ่านค่าปัจจุบัน
    current_config |= gpio_pin;  // เปิดใช้งานขาที่กำหนด
    TLA2528_Set_GPIO_Config(current_config);
}

void TLA2528_Disable_GPIO(uint8_t gpio_pin) {
    uint8_t current_config = TLA2528_Read_GPIO_Config(); // อ่านค่าปัจจุบัน
    current_config &= ~gpio_pin;  // ปิดใช้งานขาที่กำหนด
    TLA2528_Set_GPIO_Config(current_config);
}

/*
 * on / off gpio
 * GPO_DRIVE_CFG Register (Address = 0x9)
 */

uint8_t TLA2528_Read_GPO_Drive_Config(void) {
    return TLA2528_Read_Register(GPO_DRIVE_CFG_ADDRESS);
}
void TLA2528_Set_GPO_Drive_Config(uint8_t config) {
    TLA2528_Write_Register(GPO_DRIVE_CFG_ADDRESS, config);
    /*
     * TLA2528_Set_GPO_Drive_Config(0x0F);  // ตั้งค่า GPO Drive เป็น 0x0F (เปิดใช้งาน GPO0-GPO3)
     *
     */
}
void TLA2528_Enable_GPO_Drive(uint8_t gpo_pin) {
    uint8_t current_config = TLA2528_Read_GPO_Drive_Config(); // อ่านค่าปัจจุบัน
    current_config |= gpo_pin;  // เปิดใช้งาน GPO Drive ที่กำหนด
    TLA2528_Set_GPO_Drive_Config(current_config);
}

void TLA2528_Disable_GPO_Drive(uint8_t gpo_pin) {
    uint8_t current_config = TLA2528_Read_GPO_Drive_Config(); // อ่านค่าปัจจุบัน
    current_config &= ~gpo_pin;  // ปิดใช้งาน GPO Drive ที่กำหนด
    TLA2528_Set_GPO_Drive_Config(current_config);
}
/*
 * TLA2528_Enable_GPO_Drive(GPO_DRIVE_CFG_GPO0);  // เปิดใช้งาน GPO0
TLA2528_Disable_GPO_Drive(GPO_DRIVE_CFG_GPO0); // ปิดใช้งาน GPO0
 *
 */

/*GPO_VALUE Register (Address = 0xB)
 *
 */
uint8_t Read_GPO_Value(void) {
    return TLA2528_Read_Register(GPO_VALUE_ADDRESS);
}

void Set_GPO_Value(uint8_t value) {
    TLA2528_Write_Register(GPO_VALUE_ADDRESS, value);
}
void Set_GPO_Pin_High(uint8_t gpo_pin) {
    uint8_t current_value = Read_GPO_Value();      // อ่านค่า GPO VALUE ปัจจุบัน
    current_value |= gpo_pin;                      // ตั้งค่า GPO pin ที่ระบุให้เป็น High
    Set_GPO_Value(current_value);                  // ส่งค่ากลับไปที่ GPO_VALUE Register
}

void Set_GPO_Pin_Low(uint8_t gpo_pin) {
    uint8_t current_value = Read_GPO_Value();      // อ่านค่า GPO VALUE ปัจจุบัน
    current_value &= ~gpo_pin;                     // ตั้งค่า GPO pin ที่ระบุให้เป็น Low
    Set_GPO_Value(current_value);                  // ส่งค่ากลับไปที่ GPO_VALUE Register
}
void Set_All_GPOs_High(void) {
    Set_GPO_Value(0xFF);  // ตั้งค่าทุก GPO pin ให้เป็น High
}
void Set_All_GPOs_Low(void) {
    Set_GPO_Value(0x00);  // ตั้งค่าทุก GPO pin ให้เป็น Low
}
